local L = AceLibrary("AceLocale-2.2"):new("PortalFu")

L:RegisterTranslations("enUS", function() return
{
	HEARTHSTONE = "Hearthstone",
	INN = "Inn:",
	MIN = "mins",
	["N/A"] = "N/A",
	P_RUNE = "Rune of Portals",
	RCLICK = "Right-Click",
	READY = "Ready",
	SCROLL_3 = "Scroll of Recall III",
	SEE_SPELLS = "to see list of spells",
	TP_P = "Teleports / Portals",
	TP_RUNE = "Rune of Teleportation",
	["Astral Recall"] = true,
	["Portal: Darnassus"] = true,
	["Portal: Ironforge"] = true,
	["Portal: Orgrimmar"] = true,
	["Portal: Stormwind"] = true,
	["Portal: Thunder Bluff"] = true,
	["Portal: Undercity"] = true,
	["Portal: Stonard"] = true,
	["Portal: Theramore"] = true,
	["Portal: Alah'Thalas"] = true,
	["Teleport: Darnassus"] = true,
	["Teleport: Ironforge"] = true,
	["Teleport: Moonglade"] = true,
	["Teleport: Orgrimmar"] = true,
	["Teleport: Stormwind"] = true,
	["Teleport: Thunder Bluff"] = true,
	["Teleport: Undercity"] = true,
	["Teleport: Stonard"] = true,
	["Teleport: Theramore"] = true,
	["Teleport: Alah'Thalas"] = true,
}

end)

L:RegisterTranslations("ruRU", function() return
{
	HEARTHSTONE = "Камень возвращения",
	INN = "Таверна:",
	MIN = "мин",
	["N/A"] = "N/A",
	P_RUNE = "Руна порталов",
	RCLICK = "ПКМ",
	READY = "Готово",
	SCROLL_3 = "Свиток возврата III",
	SEE_SPELLS = "Для открытия списка заклинаний",
	TP_P = "Телепорты / Порталы",
	TP_RUNE = "Руна телепортации",
	["Astral Recall"] = "Астральное возвращение",
	["Portal: Darnassus"] = "Портал: Дарнас",
	["Portal: Ironforge"] = "Портал: Стальгорн",
	["Portal: Orgrimmar"] = "Портал: Оргриммар",
	["Portal: Stormwind"] = "Портал: Штормград",
	["Portal: Thunder Bluff"] = "Портал: Громовой Утес",
	["Portal: Undercity"] = "Портал: Подгород",
	["Portal: Stonard"] = "Портал: Каменор",
	["Portal: Theramore"] = "Портал: Терамор",
	["Portal: Alah'Thalas"] = "Портал: Alah'Thalas",
	["Teleport: Darnassus"] = "Телепорт: Дарнас",
	["Teleport: Ironforge"] = "Телепорт: Стальгорн",
	["Teleport: Moonglade"] = "Телепорт: Лунная поляна",
	["Teleport: Orgrimmar"] = "Телепорт: Оргриммар",
	["Teleport: Stormwind"] = "Телепорт: Штормград",
	["Teleport: Thunder Bluff"] = "Телепорт: Громовой Утес",
	["Teleport: Undercity"] = "Телепорт: Подгород",
	["Teleport: Stonard"] = "Телепорт: Каменор",
	["Teleport: Theramore"] = "Телепорт: Терамор",
	["Teleport: Alah'Thalas"] = "Телепорт: Alah'Thalas",
}

end)

L:RegisterTranslations("esES", function() return
{
  HEARTHSTONE = "Piedra de hogar",
	INN = "Posada:",
	MIN = "min",
	["N/A"] = "N/D",
	P_RUNE = "Runa de portales",
	RCLICK = "Clic derecho",
	READY = "Listo",
	SCROLL_3 = "Pergamino de regreso III",
	SEE_SPELLS = "para ver la lista de hechizos",
	TP_P = "Teletransportes / Portales",
	TP_RUNE = "Runa de teletransporte",
	["Astral Recall"] = true,
  ["Portal: Darnassus"] = true,
  ["Portal: Ironforge"] = true,
  ["Portal: Orgrimmar"] = true,
  ["Portal: Stormwind"] = true,
  ["Portal: Thunder Bluff"] = true,
  ["Portal: Undercity"] = true,
  ["Portal: Stonard"] = true,
  ["Portal: Theramore"] = true,
  ["Portal: Alah'Thalas"] = true,
  ["Teleport: Darnassus"] = true,
  ["Teleport: Ironforge"] = true,
  ["Teleport: Moonglade"] = true,
  ["Teleport: Orgrimmar"] = true,
  ["Teleport: Stormwind"] = true,
  ["Teleport: Thunder Bluff"] = true,
  ["Teleport: Undercity"] = true,
  ["Teleport: Stonard"] = true,
  ["Teleport: Theramore"] = true,
  ["Teleport: Alah'Thalas"] = true,
}

end)

L:RegisterTranslations("deDE", function() return
{
	HEARTHSTONE = "Ruhestein",
	INN = "Gasthaus:",
	MIN = "Min.",
	["N/A"] = "N/A",
	P_RUNE = "Rune der Portale",
	RCLICK = "Rechtsklick",
	READY = "Bereit",
	SCROLL_3 = "Rolle des Rückrufs III",
	SEE_SPELLS = "für Liste der Portalzauber",
	TP_P = "Teleportationen / Portale",
	TP_RUNE = "Rune der Teleportation",
	["Astral Recall"] = "Astraler Rückruf",
	["Portal: Darnassus"] = "Portal: Darnassus",
	["Portal: Ironforge"] = "Portal: Ironforge",
	["Portal: Orgrimmar"] = "Portal: Orgrimmar",
	["Portal: Stormwind"] = "Portal: Stormwind",
	["Portal: Thunder Bluff"] = "Portal: Thunder Bluff",
	["Portal: Undercity"] = "Portal: Undercity",
	["Portal: Stonard"] = "Portal: Stonard",
	["Portal: Theramore"] = "Portal: Theramore",
	["Portal: Alah'Thalas"] = "Portal: Alah'Thalas",
	["Teleport: Darnassus"] = "Teleport: Darnassus",
	["Teleport: Ironforge"] = "Teleport: Ironforge",
	["Teleport: Moonglade"] = "Teleport: Moonglade",
	["Teleport: Orgrimmar"] = "Teleport: Orgrimmar",
	["Teleport: Stormwind"] = "Teleport: Stormwind",
	["Teleport: Thunder Bluff"] = "Teleport: Thunder Bluff",
	["Teleport: Undercity"] = "Teleport: Undercity",
	["Teleport: Stonard"] = "Teleport: Stonard",
	["Teleport: Theramore"] = "Teleport: Theramore",
	["Teleport: Alah'Thalas"] = "Teleport: Alah'Thalas",
}

end)

L:RegisterTranslations("koKR", function() return
{
	HEARTHSTONE = "귀환석",
	INN = "여관:",
	MIN = "분",
	["N/A"] = "사용 불가",
	P_RUNE = "차원이동의 룬",
	RCLICK = "오른쪽-클릭",
	READY = "사용 가능",
	SCROLL_3 = "Scroll of Recall III", -- Requires localization
	SEE_SPELLS = "주문 목록 보기",
	TP_P = "순간이동 / 차원의 문",
	TP_RUNE = "순간이동의 룬",
}

end)

L:RegisterTranslations("zhTW", function() return
{
	HEARTHSTONE = "爐石",
	INN = "旅店：",
	MIN = "分",
	["N/A"] = "N/A",
	P_RUNE = "傳送門符文",
	RCLICK = "右鍵點擊",
	READY = "準備", -- Needs review
	SCROLL_3 = "召回卷軸 III",
	SEE_SPELLS = "to see list of spells", -- Needs review
	TP_P = "傳送 / 傳送門",
	TP_RUNE = "傳送符文",
}

end)

L:RegisterTranslations("zhCN", function() return
{
	HEARTHSTONE = "炉石",
	INN = "旅店：",
	MIN = "分",
	["N/A"] = "N/A",
	P_RUNE = "传送门符文",
	RCLICK = "右键点击",
	READY = "准备", -- Needs review
	SCROLL_3 = "召回卷轴 III",
	SEE_SPELLS = "to see list of spells", -- Needs review
	TP_P = "传送 / 传送门",
	TP_RUNE = "传送符文",
}

end)